...
<script type="text/javascript">
    ...
    function fetchAccessoryBrandList() {
        //var brand = "nn";
        //alert(st);
        var ajaxRequest = new GetXmlHttpObject();
        ajaxRequest.open('POST', 'AccesoryBrandServlet?service=RetreiveModelList', true);
        ...
        ajaxRequest.onreadystatechange = function () {
            if (ajaxRequest.readyState == 4) {
                if (ajaxRequest.status == 200) {
                    ...
                    if (resultString != '') {
                        var targetOptions = document.getElementsByName('accessoribrandId')[0].options;

                        if (targetOptions.length == 1 && targetOptions.value != "") {
                            targetOptions.length = 0;
                        }
                        document.getElementById('accessoribrandId').innerHTML = "";
...
<script type="text/javascript">
    ...
    function fetchAccessoryTypeList() {
        //var brand = "nn";
        //alert(st);
        var ajaxRequest = new GetXmlHttpObject();
        ajaxRequest.open('POST', 'AccesoryServlet?service=RetreiveModelList', true);
        ajaxRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        ajaxRequest.onreadystatechange = function () {
            if (ajaxRequest.readyState == 4) {
                if (ajaxRequest.status == 200) {
                    var resultString = ajaxRequest.responseText;
                    if (resultString != '') {
                        var targetOptions = document.getElementsByName('accessorytypeId')[0].options;

                        if (targetOptions.length == 1 && targetOptions.value != "") {
                            targetOptions.length = 0;
                        }
                        document.getElementById('accessorytypeId').innerHTML = "";
                        var resultStringSplitArr = new Array();
                        resultStringSplitArr = resultString.split('#');
                        //alert(resultStringSplitArr);
